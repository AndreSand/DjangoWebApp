...
def post_publish(request, pk):
    post = get_object_or_404(Post, pk=pk)
    post.publish()
    return redirect('MyWebApp.views.post_detail', pk=pk)

@login_required
def post_remove(request, pk):
    post = get_object_or_404(Post, pk=pk)
    post.delete()
    return redirect('MyWebApp.views.post_list')
...
def post_draft_list(request):
    ...
    return render(request, 'MyWebApp/post_draft_list.html', {'posts': posts})
